# VAV: This has been tested with transformers==4.23.1 in a Docker environment
...
def convert(input_checkpoint_path, loaded_config, output_checkpoint_path):
    ...
    for layer_i in tqdm(range(get_key(loaded_config, "num-layers"))):
        ...
        tp_keys = list(loaded_tp_ranks[0].keys())
        ...
        # HF GPT-NeoX has these values are registered buffers
    ...
    tmp = [t["final_linear.weight"] for t in loaded_tp_ranks]
    if tmp[0].shape[0] * len(loaded_tp_ranks) != hf_model.embed_out.weight.shape[0]:
        cat_dim=1
    else:
        cat_dim=0
    ...
    hf_model.embed_out.load_state_dict(
        {
            "weight": torch.cat(tmp, dim=cat_dim),
        }
    )
